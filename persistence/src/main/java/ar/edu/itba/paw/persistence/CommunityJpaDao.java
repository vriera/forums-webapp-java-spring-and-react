...
public class CommunityJpaDao implements CommunityDao {
    ...
    private List<Long> publicList(Integer limit, Integer page) {
        ...
        @SuppressWarnings("unchecked") final List<Long> ids = (List<Long>) nativeQuery.getResultList().stream().map(e -> Long.valueOf(e.toString())).collect(Collectors.toList());
        return ids;
    }
    ...
    public List<Community> getCommunitiesByAccessType(Long userId, AccessType type, Integer page, Integer limit) {
        ...
        @SuppressWarnings("unchecked") final List<Integer> communityIds = (List<Integer>) nativeQuery.getResultList();
