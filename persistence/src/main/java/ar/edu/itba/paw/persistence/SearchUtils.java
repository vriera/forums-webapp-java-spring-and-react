...
public final class SearchUtils {
    ...
    public static final String RAW_SELECT = "SELECT distinct coalesce(votes , 0 ) as votes , question.question_id, question.image_id , time, title, body , users.user_id, users.username AS user_name, users.email AS user_email, users.password as user_password,\n"
            +
    ...
    private static final String MAPPED_ANSWER_QUERY = "(select question_id , " +
    ...
    public static final String MAPPED_QUERY = "SELECT distinct coalesce(votes , 0 ) as votes , question.question_id, question.image_id , time, title, body, total_answers , users.user_id, users.username AS user_name, users.email AS user_email, users.password as user_password, "
            +
            "community.community_id, community.name AS community_name, community.description, community.moderator_id, "
            +
            " forum.forum_id, forum.name AS forum_name , query , ans_rank  " +
            ...
            +
            MAPPED_ANSWER_QUERY
            + " on  question.question_id = aux_answers.question_id left join (select question_id ,count(*) as total_answers from answer group by question_id) as aux2 on aux2.question_id = question.question_id ";
    ...
    public static void appendOrder(StringBuilder mappedQuery, Number order, boolean hasText) {
        ...
        switch (order.intValue()) {
            ...
            case 2:
                if (hasText) {
                    mappedQuery.append(
                            "ts_rank_cd(to_tsvector('spanish',title), query,32) + ts_rank_cd(to_tsvector('spanish',body), query,32) DESC ");
                } else {
                    mappedQuery.append(" time DESC ");
                }
                break;
