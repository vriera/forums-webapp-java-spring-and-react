...
const RequestedCard = (props: {user: User, acceptRequestCallback: () => void, rejectRequestCallback: () => void, blockCommunityCallback: () => void}) => {
                ...
                <div className="d-flex flex-row mt-3" style={{justifyContent: "space-between"}}>
                    ...
                    <div className="row">
                        <div className="col-auto mx-0 px-0">
                            <button className="btn mb-0" onClick={props.acceptRequestCallback} title={t("dashboard.AcceptRequest")} >
                        ...
                        <div className="col-auto mx-0 px-0">
                            <button className="btn mb-0" onClick={props.rejectRequestCallback} title={t("dashboard.RejectRequest")}>
                                <div className="h4 mb-0">
                        ...
                        <div className="col-auto mx-0 px-0">
                            <button className="btn mb-0" onClick={props.blockCommunityCallback} title={t("dashboard.BlockCommunity")}>
                                <div className="h4 mb-0">
      ...
      <>
        ...
        <div className="overflow-auto">
            ...
            props.params.userList.map((user: User) =>
              <>
                <ModalPage buttonName={t("dashboard.AcceptRequest")} show={showModalForAccept} onClose={handleCloseModalForAccept} onConfirm={() => handleAccept(user.id)}/>
                <ModalPage buttonName={t("dashboard.BlockCommunity")} show={showModalForBlock} onClose={handleCloseModalForBlock} onConfirm={() => handleBlock(user.id)}/>
                <ModalPage buttonName={t("dashboard.RejectRequest")} show={showModalForReject} onClose={handleCloseModalForReject} onConfirm={() => handleReject(user.id)}/>

                <RequestedCard user={user} key={user.id} acceptRequestCallback={handleShowForAccept} rejectRequestCallback={handleShowForReject} blockCommunityCallback={handleShowForBlock}/>
              </>
            )
            }
            ...
            {props.params.userList && props.params.userList.length === 0 && (
              ...
              <div className="ml-5">
                  <p className="row h1 text-gray">{t("dashboard.noPendingRequests")}</p>
...
const RequestedUsersPane = (props: {params: UserContentType}) => {
    ...
    return (
      <div className="white-pill mt-5">
        ...
        <DashboardCommunitiesTabs activeTab="requested" communityId={props.params.selectedCommunity.id} communityPage={props.params.currentCommunityPage}/>
...
const RequestedUsersPage = () => {
  ...
  useEffect(() => {
    ...
    history.push({ pathname: `${process.env.PUBLIC_URL}/dashboard/communities/${communityId}/requested?communityPage=${communityPageFromQuery}&userPage=${userPageFromQuery}`})
  ...
  function setCommunityPageCallback(page: number): void{
    history.push({ pathname: `${process.env.PUBLIC_URL}/dashboard/communities/${communityId}/requested?communityPage=${page}&userPage=${userPage}`})
    setCommunityPage(page);
    setModeratedCommunities(undefined);
  }

  function setSelectedCommunityCallback(community: CommunityCard): void{
    history.push({ pathname: `${process.env.PUBLIC_URL}/dashboard/communities/${community.id}/requested?communityPage=${communityPage}&userPage=${userPage}`})
  ...
  function setUserPageCallback(page: number): void{
    history.push({ pathname: `${process.env.PUBLIC_URL}/dashboard/communities/${communityId}/requested?communityPage=${communityPage}&userPage=${page}`})
    setUserPage(page);
