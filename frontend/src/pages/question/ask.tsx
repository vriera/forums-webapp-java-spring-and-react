...
const AskQuestionMainCard = (props: {title: string, currentProgress: number, incrementProgress:any, decrementProgress:any}) => {
        ...
        <div className="container">
            <div className="white-pill">
                ...
                {props.currentProgress === 1 &&
                    <SelectCommunity communityList={[community1, community2]} incrementProgress={props.incrementProgress}/>
                }
                {/* If currentProgress is 2 show askQuestionContent */}
                {props.currentProgress === 2 &&
                    <AskQuestionContent incrementProgress={props.incrementProgress} decrementProgress={props.decrementProgress}/>
                }
                ...
                <div className="stepper-wrapper">
					...
					<div className={"stepper-item " + (props.currentProgress ==1 && " active ") + ((props.currentProgress >1) && " completed") }>
						<div className="step-counter">1</div>
						<div className="step-name">{t("question.community")}</div>
					</div>
					<div className= {"stepper-item " + (props.currentProgress === 2 && " active ") + ((props.currentProgress > 2) && " completed")}>
						<div className="step-counter">2</div>
						<div className="step-name">{t("question.content")}</div>
					</div>
					<div className= {"stepper-item " + (props.currentProgress === 3 && " active") }>
...
const SelectCommunity = (props:{communityList: Community[], incrementProgress: any} ) => {
    const { t } = useTranslation();
    return (
        <>
            ...
            <div className="container">
                {props.communityList.map(community => 
                    <button onClick={() => props.incrementProgress()} className="btn btn-outline-primary badge-pill badge-lg my-3">{community.name}</button>
                )}
            </div>
        </>
    )
...
const AskQuestionContent = (props: {incrementProgress: any, decrementProgress: any}) => {
    const { t } = useTranslation();
    return (
        <>
            ...
            <div className="d-flex justify-content-center">
                <button onClick={() => props.decrementProgress()} className="btn btn-light align-self-start" >{t("profile.back")}</button>
                <button onClick={() => props.incrementProgress()} className="btn btn-primary mb-3" type="submit">{t("button.continue")}</button>
            </div>
        </>
    )
}
...
const AskQuestionPage = () => {
    ...
    return (
        <div className="section section-hero section-shaped">
            <Background/>
            <AskQuestionMainCard title={t("community.select")} currentProgress={progress} incrementProgress={incrementProgress} decrementProgress={decrementProgress}/>
        </div>
    )
}
