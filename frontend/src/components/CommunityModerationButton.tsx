...
const CommunityModerationButton = (props: {community: CommunityCard, selectedCommunity: CommunityCard, setSelectedCommunityCallback: (community: CommunityCard) => void }) => {
      ...
      //  useEffect( () => {} , [selectedCommunity])
        ...
        return (
            <div className="row">
                <button onClick={() => setSelectedCommunity(community)} className={"btn  badge-pill badge-lg my-3 " + (community.id !== props.selectedCommunity.id?  "btn-outline-primary":"") + (community.id === props.selectedCommunity.id? "btn-primary ":"")}>{community.name}   
